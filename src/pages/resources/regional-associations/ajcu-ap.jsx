...
function AjcuApAssociation() {
    ...
    useEffect(() => {
        async function loadVideos() {
            ...
            if (videoTitle != null) {
                res = await getAllRegionalAssociations({ isAdmin: false, page: page, videoRegion: 'ajcu-ap', videoTitle: videoTitle })
            }
            else if (videoTitle == null) {
                res = await getAllRegionalAssociations({ isAdmin: false, page: page, videoRegion: 'ajcu-ap' })
            }
    ...
    async function submitFilter(e) {
        ...
        if (selectFilter == "all") {
            window.history.replaceState(
                "",
                "",
                `/resources/regional-associations/ajcu-ap?page=1`
            );
        } else if (selectFilter == "videoTitle") {
            window.history.replaceState(
                "",
                "",
                `/resources/regional-associations/ajcu-ap?videoTitle=${textFilter}&page=1`
            );
        }
        ...
        <section className='min-h-screen'>
            ...
            <div className='container mx-auto'>
                <ul className='flex gap-5 [&>*]:text-[1rem] [&>*]:mb-2 overflow-x-auto'>
                    ...
                    <li className='font-semibold border-b-2 border-black dark:border-white'><Link to='/resources/regional-associations/ajcu-ap'>AJCU-AP</Link></li>
                ...
                <div className='flex items-start justify-center w-full my-10'>
                    <img src="/media/images/regional_associations/ajcu-ap.png" alt="" className='max-w-[40rem]' />
                </div>
                ...
                {filteredPagination.length > 1 && <div className="mt-10">
                    <ul className="pagination">
                        <li>
                            <a
                                href={queryParameters.get("videoTitle") ? `/resources/regional-associations/ajcu-ap?videoTitle=${queryParameters.get("videoTitle")}&page=1` :
                                    queryParameters.get("id") ? `/resources/regional-associations/ajcu-ap?id=${queryParameters.get("id")}&page=1` : '/resources/regional-associations/ajcu-ap?page=1'}
                        ...
                        <li>
                            {
                                <a
                                    href={queryParameters.get("videoTitle") ? `/resources/regional-associations/ajcu-ap?videoTitle=${queryParameters.get("videoTitle")}&page=${page}` : queryParameters.get("videoRegion") ? `/resources/regional-associations/ajcu-ap?videoRegion=${queryParameters.get("videoRegion")}&page=${page}` :
                                        queryParameters.get("id") ? `/resources/regional-associations/ajcu-ap?id=${queryParameters.get("id")}&page=${page}` :
                                            (queryParameters.get("page") == '1' && queryParameters.get("videoTitle")) ?
                                                `/resources/regional-associations/ajcu-ap?videoTitle=${videoTitle}&page=${videosCount}` : `/resources/regional-associations/ajcu-ap?page=${videosCount}`}
                                    onClick={() => updatePage("subtract")}
                                    className="!p-0"
                                >
                                    <MdKeyboardArrowLeft />
                        ...
                        {filteredPagination.map((item, index) => (
                            <li key={index}>
                                {
                                    <a
                                        href={queryParameters.get("videoTitle") ? `/resources/regional-associations/ajcu-ap?videoTitle=${queryParameters.get("videoTitle")}&page=${page}` :
                                            queryParameters.get("id") ? `/resources/regional-associations?id=${queryParameters.get("id")}&page=${page}` : queryParameters.get("videoRegion") ? `/resources/regional-associations/ajcu-ap?videoRegion=${queryParameters.get("videoRegion")}&page=${page}` :
                                                (queryParameters.get("page") == '1' && queryParameters.get("videoTitle")) ?
                                                    `/resources/regional-associations/ajcu-ap?videoTitle=${videoTitle}&page=${page}` : `/resources/regional-associations/ajcu-ap?page=${page}`}
                                    ...
                                    href={queryParameters.get("videoTitle") ? `/resources/regional-associations/ajcu-ap?videoTitle=${queryParameters.get("videoTitle")}&page=${page}` : queryParameters.get("videoRegion") ? `/resources/regional-associations/ajcu-ap?videoRegion=${queryParameters.get("videoRegion")}&page=${page}` :
                                        queryParameters.get("id") ? `/resources/regional-associations/ajcu-ap?id=${queryParameters.get("id")}&page=${page}` :
                                            (queryParameters.get("page") == `${videosCount}` && queryParameters.get("videoTitle")) ?
                                                `/resources/regional-associations/ajcu-ap?videoTitle=${videoTitle}&page=1` : `/resources/regional-associations/ajcu-ap?page=1`}
                        ...
                        <li>
                            <a
                                href={queryParameters.get("videoTitle") ? `/resources/regional-associations/ajcu-ap?videoTitle=${queryParameters.get("videoTitle")}&page=${videosCount}` : queryParameters.get("videoRegion") ? `/resources/regional-associations/ajcu-ap?videoRegion=${queryParameters.get("videoRegion")}&page=1` :
                                    queryParameters.get("id") ? `/resources/regional-associations/ajcu-ap?id=${queryParameters.get("id")}&page=${videosCount}` :
                                        (queryParameters.get("page") == `${videosCount}` && queryParameters.get("videoTitle")) ?
                                            `/resources/regional-associations/ajcu-ap?videoTitle=${videoTitle}&page=${videosCount}` : `/resources/regional-associations/ajcu-ap?page=${videosCount}`}
                                onClick={() => updatePage("subtract")}
                                className="!p-0"
...
export default AjcuApAssociation