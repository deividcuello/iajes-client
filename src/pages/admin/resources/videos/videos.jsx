...
import { deleteVideo, getAllVideos, getVideo } from '../../../../api'
...
function VideosAdmin() {
    ...
    useEffect(() => {
        async function loadVideos() {
            // const res = await getAllVideos({ isAdmin: true, page: page })
            ...
            if (videoTitle != null) {
                ...
                res = await getAllVideos({ page: page, isAdmin: true });
            }
    ...
    async function submitFilter(e) {
        ...
        if (selectFilter == "all") {
            window.history.replaceState(
                "",
                "",
                `/admin/resources/videos?page=1`
            );
        } else if (selectFilter == "videoTitle") {
            window.history.replaceState(
                "",
                "",
                `/admin/resources/videos?videoTitle=${textFilter}&page=1`
            );
        } else if (selectFilter == "id") {
            window.history.replaceState(
                "",
                "",
                `/admin/resources/videos?id=${textFilter}&page=1`
            );
        }
    ...
    const createVideo = (event) => {
        ...
        if (action.create) {
            ...
            fetch('https://deividcuello.pythonanywhere.com/api/videos/', {
                credentials: "include",
            ...
            fetch(`https://deividcuello.pythonanywhere.com/api/videos/${action.edit}/`, {
                credentials: "include",
                headers: { "X-CSRFToken": Cookies.get("csrftoken") },
                method: "PUT",
                body: formData,
    ...
    async function editVideo(id) {
        ...
        const getVideoData = await getVideo({ id: id, page: 1 });
    ...
    async function toggleHidden(id) {
        const documentData = await getVideo({ id: id });
        ...
        fetch(`https://deividcuello.pythonanywhere.com/api/videos/${id}/`, {
            credentials: "include",
            headers: {
            ...
            {!isVideoModal && <div>
                ...
                {filteredPagination.length > 1 && <div className="mt-10">
                    <ul className="pagination">
                        <li>
                            <a
                                href={queryParameters.get("videoTitle") ? `/admin/resources/videos?videoTitle=${queryParameters.get("videoTitle")}&page=1` :
                                    queryParameters.get("id") ? `/admin/resources/videos?id=${queryParameters.get("id")}&page=1` : '/admin/resources/videos?page=1'}
                        ...
                        <li>
                            {
                                <a
                                    href={queryParameters.get("videoTitle") ? `/admin/resources/videos?videoTitle=${queryParameters.get("videoTitle")}&page=${page}` :
                                        queryParameters.get("id") ? `/admin/resources/videos?id=${queryParameters.get("id")}&page=${page}` :
                                            (queryParameters.get("page") == '1' && queryParameters.get("videoTitle")) ?
                                                `/admin/resources/videos?videoTitle=${videoTitle}&page=${videosCount}` : `/admin/resources/videos?page=${videosCount}`}
                                    onClick={() => updatePage("subtract")}
                                    className="!p-0"
                                >
                                    <MdKeyboardArrowLeft />
                            ...
                            <li key={index}>
                                {
                                    <a
                                        href={queryParameters.get("videoTitle") ? `/admin/resources/videos?videoTitle=${queryParameters.get("videoTitle")}&page=${page}` :
                                            queryParameters.get("id") ? `/admin/resources/videos?id=${queryParameters.get("id")}&page=${page}` :
                                                (queryParameters.get("page") == '1' && queryParameters.get("videoTitle")) ?
                                                    `/admin/resources/videos?videoTitle=${videoTitle}&page=${page}` : `/admin/resources/videos?page=${page}`}
                                    ...
                                    href={queryParameters.get("videoTitle") ? `/admin/resources/videos?videoTitle=${queryParameters.get("videoTitle")}&page=${page}` :
                                        queryParameters.get("id") ? `/admin/resources/videos?id=${queryParameters.get("id")}&page=${page}` :
                                            (queryParameters.get("page") == `${videosCount}` && queryParameters.get("videoTitle")) ?
                                                `/admin/resources/videos?videoTitle=${videoTitle}&page=1` : `/admin/resources/videos?page=1`}
                        ...
                        <li>
                            <a
                                href={queryParameters.get("videoTitle") ? `/admin/resources/videos?videoTitle=${queryParameters.get("videoTitle")}&page=${videosCount}` :
                                    queryParameters.get("id") ? `/admin/resources/videos?id=${queryParameters.get("id")}&page=${videosCount}` :
                                        (queryParameters.get("page") == `${videosCount}` && queryParameters.get("videoTitle")) ?
                                            `/admin/resources/videos?videoTitle=${videoTitle}&page=${videosCount}` : `/admin/resources/videos?page=${videosCount}`}
                                onClick={() => updatePage("subtract")}
                                className="!p-0"
