...
function Water() {
  ...
  async function submitFilter(e) {
    ...
    if (selectFilter == "all") {
      ...
      window.history.replaceState(
        "",
        "",
        `${baseWaterUrl}?university=${textFilter.replace(/\s+/g, ' ').trim()}&page=1`
      );
    } else if (selectFilter == "association") {
      window.history.replaceState(
        "",
        "",
        `${baseWaterUrl}?association=${textFilter.replace(/\s+/g, ' ').trim()}&page=1`
      );
    } else if (selectFilter == "keyword") {
      window.history.replaceState(
        "",
        "",
        `${baseWaterUrl}?keyword=${textFilter.replace(/\s+/g, ' ').trim()}&page=1`
      );
    } else if (selectFilter == "start_year") {
      window.history.replaceState(
        "",
        "",
        `${baseWaterUrl}?start_year=${textFilter.replace(/\s+/g, ' ').trim()}&page=1`
      );
    } else if (selectFilter == "end_year") {
      window.history.replaceState(
        "",
        "",
        `${baseWaterUrl}?end_year=${textFilter.replace(/\s+/g, ' ').trim()}&page=1`
      );
    } else if (selectFilter == "investigator") {
      window.history.replaceState(
        "",
        "",
        `${baseWaterUrl}?investigator=${textFilter.replace(/\s+/g, ' ').trim()}&page=1`
      );
    }
  ...
  async function submitSearchFilter(e) {
    ...
    if (textSearchFilter.length > 0) {
      window.history.replaceState(
        "",
        "",
        `${baseWaterUrl}?projectName=${textSearchFilter.replace(/\s+/g, ' ').trim()}&page=1`
      );
    } else {
      window.history.replaceState(
        "",
  ...
  return (
    <section>
      <div>
        <div className="w-full h-[30rem] bg-[url('/media/images/projects/water/hero.webp')] bg-no-repeat bg-cover bg-center text-white flex items-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-10 mt-10">
            {
              projectsData.map((project, index) => (
                <Link key={index} to={`/projects/water/${project.slug}`}>
                  <div className="h-52">
        ...
        {filteredPagination.length > 1 && <div className="mt-10">
          <ul className="pagination">
            <li>
              <a
                href={queryParameters.get("keyword") ? `/projects/water?keyword=${queryParameters.get("keyword")}&page=1` : queryParameters.get("projectName") ? `/projects/water?projectName=${queryParameters.get("projectName")}&page=${page}` : queryParameters.get("university") ? `/projects/water?university=${queryParameters.get("university")}&page=1` :
                  queryParameters.get("association") ? `/projects/water?association=${queryParameters.get("association")}&page=1` : queryParameters.get("start_year") ? `/projects/water?start_year=${queryParameters.get("start_year")}&page=1` : queryParameters.get("end_year") ? `/projects/water?end_year=${queryParameters.get("end_year")}&page=1` : queryParameters.get("investigator") ? `/projects/water?investigator=${queryParameters.get("investigator")}&page=1`
                    : '/projects/water?page=1'
                }
            ...
            <li>
              {
                <a
                  href={queryParameters.get("keyword") ? `/projects/water?keyword=${queryParameters.get("keyword")}&page=${page}` : queryParameters.get("projectName") ? `/projects/water?projectName=${queryParameters.get("projectName")}&page=${page}` : queryParameters.get("university") ? `/projects/water?university=${queryParameters.get("university")}&page=${page}` :
                    queryParameters.get("association") ? `/projects/water?association=${queryParameters.get("association")}&page=${page}` : queryParameters.get("start_year") ? `/projects/water?start_year=${queryParameters.get("start_year")}&page=${page}` : queryParameters.get("end_year") ? `/projects/water?end_year=${queryParameters.get("end_year")}&page=${page}` : queryParameters.get("investigator") ? `/projects/water?investigator=${queryParameters.get("investigator")}&page=${page}`
                      : `/projects/water?page=${page}`
                  }
                  onClick={() => updatePage("subtract")}
                  className="!p-0"
                >
            ...
            <li>
              <a
                href={queryParameters.get("keyword") ? `/projects/water?keyword=${queryParameters.get("keyword")}&page=${projectsCount}` : queryParameters.get("projectName") ? `/projects/water?projectName=${queryParameters.get("projectName")}&page=${projectsCount}` : queryParameters.get("university") ? `/projects/water?university=${queryParameters.get("university")}&page=${projectsCount}` :
                  queryParameters.get("association") ? `/projects/water?association=${queryParameters.get("association")}&page=${projectsCount}` : queryParameters.get("start_year") ? `/projects/water?start_year=${queryParameters.get("start_year")}&page=${projectsCount}` : queryParameters.get("end_year") ? `/projects/water?end_year=${queryParameters.get("end_year")}&page=${projectsCount}` : queryParameters.get("investigator") ? `/projects/water?investigator=${queryParameters.get("investigator")}&page=${projectsCount}`
                    : `/projects/water?page=${projectsCount}`
                }
                onClick={() => updatePage("subtract")}
                className="!p-0"
              >
