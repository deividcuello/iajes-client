...
function Farming() {
  ...
  async function submitFilter(e) {
    ...
    if (selectFilter == "all") {
      window.history.replaceState(
        "",
        "",
        `${baseFarmingUrl}?page=1`
      );
    } else if (selectFilter == "university") {
      window.history.replaceState(
        "",
        "",
        `${baseFarmingUrl}?university=${textFilter.replace(/\s+/g, ' ').trim()}&page=1`
      );
    } else if (selectFilter == "association") {
      window.history.replaceState(
        "",
        "",
        `${baseFarmingUrl}?association=${textFilter.replace(/\s+/g, ' ').trim()}&page=1`
      );
    } else if (selectFilter == "keyword") {
      window.history.replaceState(
        "",
        "",
        `${baseFarmingUrl}?keyword=${textFilter.replace(/\s+/g, ' ').trim()}&page=1`
      );
    } else if (selectFilter == "start_year") {
      window.history.replaceState(
        "",
        "",
        `${baseFarmingUrl}?start_year=${textFilter.replace(/\s+/g, ' ').trim()}&page=1`
      );
    } else if (selectFilter == "end_year") {
      window.history.replaceState(
        "",
        "",
        `${baseFarmingUrl}?end_year=${textFilter.replace(/\s+/g, ' ').trim()}&page=1`
      );
    } else if (selectFilter == "investigator") {
      window.history.replaceState(
        "",
        "",
        `${baseFarmingUrl}?investigator=${textFilter.replace(/\s+/g, ' ').trim()}&page=1`
      );
    }
  ...
  async function submitSearchFilter(e) {
    ...
    if (textSearchFilter.length > 0) {
      window.history.replaceState(
        "",
        "",
        `${baseFarmingUrl}?projectName=${textSearchFilter.replace(/\s+/g, ' ').trim()}&page=1`
      );
    } else {
      window.history.replaceState(
        "",
  ...
  return (
    <section>
      <div>
        <div className="w-full h-[30rem] bg-[url('/media/images/projects/farming/hero.jpg')] bg-no-repeat bg-cover bg-center text-white flex items-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-10 mt-10">
            {
              projectsData.map((project, index) => (
                <Link key={index} to={`/projects/farming/${project.slug}`}>
                  <div className="h-52">
      ...
      {filteredPagination.length > 1 && <div className="mt-10">
        <ul className="pagination">
          <li>
            <a
              href={queryParameters.get("keyword") ? `/projects/farming?keyword=${queryParameters.get("keyword")}&page=1` : queryParameters.get("projectName") ? `/projects/farming?projectName=${queryParameters.get("projectName")}&page=1` : queryParameters.get("university") ? `/projects/farming?university=${queryParameters.get("university")}&page=1` :
                queryParameters.get("association") ? `/projects/farming?association=${queryParameters.get("association")}&page=1` : queryParameters.get("start_year") ? `/projects/farming?start_year=${queryParameters.get("start_year")}&page=1` : queryParameters.get("end_year") ? `/projects/farming?end_year=${queryParameters.get("end_year")}&page=1` : queryParameters.get("investigator") ? `/projects/farming?investigator=${queryParameters.get("investigator")}&page=1`
                  : `/projects/farming?page=1`
              }
          ...
          <li>
            {
              <a
                href={queryParameters.get("keyword") ? `/projects/farming?keyword=${queryParameters.get("keyword")}&page=${page}` : queryParameters.get("projectName") ? `/projects/farming?projectName=${queryParameters.get("projectName")}&page=${page}` : queryParameters.get("university") ? `/projects/farming?university=${queryParameters.get("university")}&page=${page}` :
                  queryParameters.get("association") ? `/projects/farming?association=${queryParameters.get("association")}&page=${page}` : queryParameters.get("start_year") ? `/projects/farming?start_year=${queryParameters.get("start_year")}&page=${page}` : queryParameters.get("end_year") ? `/projects/farming?end_year=${queryParameters.get("end_year")}&page=${page}` : queryParameters.get("investigator") ? `/projects/farming?investigator=${queryParameters.get("investigator")}&page=${page}`
                    : `/projects/farming?page=${page}`
                }
                className="!p-0"
              >
                <MdKeyboardArrowLeft />
          ...
          <li>
            <a
              href={queryParameters.get("keyword") ? `/projects/farming?keyword=${queryParameters.get("keyword")}&page=${projectsCount}` : queryParameters.get("projectName") ? `/projects/farming?projectName=${queryParameters.get("projectName")}&page=${projectsCount}` : queryParameters.get("university") ? `/projects/farming?university=${queryParameters.get("university")}&page=${projectsCount}` :
                queryParameters.get("association") ? `/projects/farming?association=${queryParameters.get("association")}&page=${projectsCount}` : queryParameters.get("start_year") ? `/projects/farming?start_year=${queryParameters.get("start_year")}&page=${projectsCount}` : queryParameters.get("end_year") ? `/projects/farming?end_year=${queryParameters.get("end_year")}&page=${projectsCount}` : queryParameters.get("investigator") ? `/projects/farming?investigator=${queryParameters.get("investigator")}&page=${projectsCount}`
                  : `/projects/farming?page=${projectsCount}`
              }
              onClick={() => updatePage("subtract")}
              className="!p-0"
            >
